...
namespace lslidar_driver {
    ...
    bool LslidarDriver::initialize() {
        this->initTimeStamp();
        if (!loadParameters()) {
            std::cerr << "Cannot load all required ROS parameters." << std::endl;
            //ROS_INFO("cannot load all required ROS parameters.");
            return false;
        }
        if (!createRosIO()) {
            std::cerr << "Cannot create all ROS IO." << std::endl;
            ...
            if (rc == 0) {
                ...
                if (difop_packet_ptr->data[1196] == 0x03 || (difop_packet_ptr->data[1196] == 0x02 && difop_packet_ptr->data[1197] /16 == 8)) {
                    ...
                    if (is_print_working_time) {
                        ...
                        printf("total working time: %d hours:%d minutes", total_working_time / 60,
                                    total_working_time % 60);
                        //ROS_INFO("total working time: %d hours:%d minutes", total_working_time / 60,
                        //            total_working_time % 60);
                        printf("less than     -40 degrees  working time: %d hours:%d minutes",
                        ...
                        //            less_minus40_degree_working_time / 60, less_minus40_degree_working_time % 60);
                        printf("-40 degrees ~ -10 degrees  working time: %d hours:%d minutes",
                        ...
                        //            positive30_to_70_working_time / 60, positive30_to_70_working_time % 60);
                        printf("70  degrees ~ 100 degrees  working time: %d hours:%d minutes",
                ...
                if(fpga_type == 0 ) {
                    std::cerr << "Unknown lidar version" << std::endl;
...
//        output_scan.scan_time = scan_time_;
        ...
        for (sensor_msgs::PointCloud2Iterator<float> iter_x(cloud_msg, "x"), iter_y(cloud_msg, "y"),
            ...
            if (range < min_range) {
                printf("rejected for range %f below minimum value %f. Point: (%f, %f, %f)", range, min_range,
                          *iter_x,
                          *iter_y, *iter_z);
            ...
            if (range > max_range) {
                printf("rejected for range %f above maximum value %f. Point: (%f, %f, %f)", range, max_range,
                          *iter_x,
                          *iter_y, *iter_z);
    ...
    /* bool LslidarDriver::powerOn(lslidar_cx_driver::lslidar_control::Request &req,
        ...
        if (!is_get_difop_) {
            res.result = 0;
            std::cerr << "Can not get dev packet! Set failed!" << std::endl;
        ...
        if (req.laser_control == 1) {
            if ((rc_msop = msop_input_->getPacket(packet0)) == 0) {
                res.result = 1;
                std::cerr << "receive cmd:"<<req.laser_control<< ",already power on status"
                //ROS_WARN("receive cmd: %d,already power on status", req.laser_control);
                return true;
            }
            std::cerr << "receive cmd: "<<req.laser_control<<",power on"
            ...
            std::chrono::duration<double> now_in_seconds = now.time_since_epoch();
            double time1 = now_in_seconds.count();

            do {
                rc_msop = msop_input_->getPacket(packet0);
                now = std::chrono::system_clock::now();
                now_in_seconds = now.time_since_epoch();
                double time2 = now_in_seconds.count();
                if (time2 - time1 > 20) {
                    res.result = 0;
                    std::cerr << "lidar connect error"
            ...
            std::cerr << "receive cmd: "<<req.laser_control<<",power off";
    ...
    bool LslidarDriver::timeService(lslidar_cx_driver::time_service::Request &req,
        ...
        printf("Start to modify lidar time service mode");
        ...
        if (time_service_mode == "gps") {
            ...
            if (!regex_match(ntp_ip, ipv4)) {
                std::cerr << "Parameter error, please check the input parameters" << std::endl;
        ...
        res.result = true;
        sendPacketTolidar(config_data);
        printf("Time service method modified successfully!");
    ...
    bool LslidarDriver::poll() {
        ...
        if (use_time_service && 4 == fpga_type) {
            if (time_service_mode_ == 1) {    //ptp授时
                ...
                timeStamp = Stamp(timestamp_s, timestamp_nsce);// s,ns
                ...
                packet_time_ns = packet->data[1206] +
                                 ...
                                 (packet->data[1208] << 16) +
                                 (packet->data[1209] << 24); //ns
                timeStamp = Stamp(packet_time_s, packet_time_ns);
                packet->stamp = timeStamp;
                current_packet_time = timeStamp.toSec();
            ...
            if ((timeStamp - timeStamp_bak).toSec() < 0.0 && (timeStamp - timeStamp_bak).toSec() > -1.0
                ...
                timeStamp = Stamp(packet_time_s + 1, packet_time_ns);
        ...
        if (end_fir_idx != SCANS_PER_PACKET) {
            ...
            boost::thread pointcloud_pub_thread([this] { publishPointcloud(); });
    ...
    bool LslidarDriver::determineLidarType(){
        ...
        //     ROS_INFO("return mode: %d", return_mode);
            ...
            printf("lidar type: C16_domestic\n");
            ...
            lidar_type = "c32_3";
            printf("lidar type: c32_3, version 3.0\n");
            //ROS_INFO("lidar type: c32, version 3.0 ");
            if (pkt->data[1204] == 0x39) return_mode = 2;
            printf("return mode: %d\n", return_mode);
            //ROS_INFO("return mode: %d", return_mode);
        } else {
            std::cerr << "Unknown lidar model,please check lidar model" << std::endl;
